...
using one of two sources:
   ...
   from the ECMWF data. The value is then stored in the keyword ``WVZ_USED``.

``WVZ_USED`` always reflects which PWV value was actually used for telluric correction and
ATRAN profile identification. The PWV source used for a particular pipeline product can be identified with the keyword
``WVZ_SRC``, which will have the value "ECMWF" or "HEADER". 
